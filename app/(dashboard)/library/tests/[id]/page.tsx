...
export default function TestDetailPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 测试标准 */}
      {((test.standards?.length ?? 0) > 0 || test.precautions) && (
        <div className="ams-card p-6">
          ...
          {test.standards && test.standards.length > 0 && (
            <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
              {test.standards.map((standard, index) => (
                <div
                  key={index}
                  className="rounded-ams border border-ams-border bg-ams-background p-4"
                >
                  <div className="mb-3 text-sm font-semibold text-ams-text-primary">
                    {standard.normName}
                  </div>
                  <div className="flex items-center gap-5 text-sm text-ams-text-secondary">
                    <div className="flex items-center gap-1.5">
                      <span>平均值</span>
                      <span className="font-medium text-ams-text-primary">{standard.mean}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span>标准差</span>
                      <span className="font-medium text-ams-text-primary">{standard.stdDev}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {test.precautions && <DetailItem label="注意事项" value={test.precautions} span />}
        </div>
      )}
