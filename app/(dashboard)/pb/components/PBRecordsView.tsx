...
import { Trophy, Plus, RefreshCw, Search, X, ArrowUp, ArrowDown, ChevronsUpDown } from 'lucide-react';
import { Button } from '@/components/ui/button';

// ---- 多条件排序 ----
type SortField = 'athlete' | 'exercise' | 'category' | 'value';
interface SortSpec {
  field: SortField;
  direction: 'asc' | 'desc';
}
...
/** 可排序表头：未排序时显示灰色双向图标，升序显示向上箭头，降序显示向下箭头；多条件排序时标注优先级序号 */
function SortableTh({
  label,
  field,
  sorts,
  ...
  const direction = isSorted ? sorts[idx].direction : null;

  return (
    <th className="whitespace-nowrap px-4 py-3 text-left ams-table-header">
      <button
        type="button"
        onClick={() => onSort(field)}
        title={
          isSorted
            ? `已按${label}${direction === 'asc' ? '升序' : '降序'}排序，点击切换方向，再次点击取消排序`
            : `点击按${label}排序（升序）`
        }
        ...
        <span className="inline-flex items-center">
          {isSorted ? (
            <>
              ...
              {sorts.length > 1 && (
                <span className="ml-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-ams-primary px-1 text-[10px] font-semibold leading-none text-white">
                  {idx + 1}
                </span>
              )}
            </>
          ) : (
            <ChevronsUpDown className="h-3.5 w-3.5 opacity-30 transition-opacity group-hover:opacity-80" />
          )}
        </span>
      </button>
    </th>
...
export default function PBRecordsView() {
  ...
  // 排序点击：三态循环 —— 首次点击升序（追加到末尾，点击顺序即优先级），再次点击切换降序，第三次点击移除恢复原始顺序
  ...
  // 加载下拉选项（加载全部运动员/练习，避免截断导致部分运动员不可选）
  useEffect(() => {
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-ams-border">
            <SortableTh label="运动员" field="athlete" sorts={sorts} onSort={handleSort} />
            <SortableTh label="训练项目" field="exercise" sorts={sorts} onSort={handleSort} />
            <SortableTh label="分类" field="category" sorts={sorts} onSort={handleSort} />
            <SortableTh label="PB 成绩" field="value" sorts={sorts} onSort={handleSort} />
    ...
    <div className="space-y-4">
      ...
      <div className="ams-card overflow-hidden">
        {error ? (
          ...
          /* 平滑过渡：加载新数据期间保留旧数据展示，仅降低透明度，避免页面闪烁 */
          <div className={`transition-opacity duration-200 ${isLoading ? 'opacity-50' : 'opacity-100'}`}>
            {/* 当前排序条件指示 */}
            {sorts.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5 border-b border-ams-border px-4 py-2 text-xs text-ams-text-muted">
                <span>排序：</span>
                {sorts.map((s, i) => (
                  <span
                    key={s.field}
                    className="inline-flex items-center gap-1 rounded-full bg-ams-surface-hover px-2 py-0.5 text-ams-text-secondary"
                  >
                    <span className="flex h-3.5 min-w-3.5 items-center justify-center rounded-full bg-ams-primary px-0.5 text-[9px] font-semibold leading-none text-white">
                      {i + 1}
                    </span>
                ...
                <button
                  type="button"
                  onClick={clearSorts}
                  className="text-ams-text-muted underline-offset-2 hover:text-ams-danger hover:underline"
                >
                  清除排序
                </button>
              </div>
