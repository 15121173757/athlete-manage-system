...
export default function NewFitnessPlanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        router.push(`/fitness/plans/${json.data.id}`);
      } else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-xl font-semibold text-ams-text-primary">新建体能测试计划</h2>
      </div>
        ...
        <div className="ams-card p-6">
          ...
          >
            ...
            <span className="text-sm font-medium text-ams-text-primary">
              全选{athleteSearch ? `（当前筛选 ${filteredAthletes.length} 人）` : ''}
            </span>
          </div>
