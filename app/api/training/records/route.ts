...
import { trainingRecordCreateSchema } from '@/lib/utils/validation';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const exerciseId = searchParams.get('exerciseId')
      ? parseInt(searchParams.get('exerciseId')!)
      : undefined;
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = trainingRecordCreateSchema.safeParse(body);
    if (!parsed.success) {
      const firstError = parsed.error.errors[0];
      throw new ValidationError(firstError?.message || '输入数据无效');
    }

    const result = await createTrainingRecord(parsed.data, user.userId);
