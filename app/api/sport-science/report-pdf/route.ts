...
/**
 * POST /api/sport-science/report-pdf
 * 运动科学工具箱通用报告 PDF 导出（服务端生成，嵌入中文字体）
 *
 * body: {
 *   title: string;             // 报告标题
 *   subtitle?: string;         // 副标题（如运动员/专项信息）
...
export async function POST(req: Request) {
  try {
    ...
    const { title, subtitle, generatedAt, sections } = body as {
      title: string;
      subtitle?: string;
      generatedAt: string;
      sections: PdfSection[];
    ...
    if (generatedAt) doc.text(generatedAt, pageWidth - marginX, 12, { align: 'right' });
    ...
    doc.setFontSize(20);
    doc.text(title || '运动科学分析报告', pageWidth / 2, y, { align: 'center' });
    y += 8;
    if (subtitle) {
      doc.setFontSize(11);
    ...
    for (let i = 1; i <= pages; i++) {
      ...
      doc.text('AMS 运动科学分析报告 · 内部资料', marginX, pageHeight - 10);
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="sport-report-${Date.now()}.pdf"`,
      },
    });
  } catch (error) {
